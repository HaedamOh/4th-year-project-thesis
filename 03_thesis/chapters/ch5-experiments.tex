...
(ii) Performance assessment during both online and offline SLAM operations within dense forest settings. \mfallon{somewhere here you need to say you tested only Logg3dNet for some experiments}
...
In this experiment, we evaluated the descriptors of four different place recognition models (Logg3dNet, EgoNN, ScanContext, STD) focusing on their ability to accurately capture loop-candidates in forest environments. Logg3dNet and EgoNN models are learning-based methods and were pre-trained on the Wild-Places dataset. Precision-recall curves (See \figref{fig:pr_curves}) show how accurately (precision) and frequently (recall) each  model detects correct loop candidates within a reasonable distance threshold (here set to 10\,m) at various descriptor thresholds $\tau_{s}$ in four different forests. \haedam{We used a Hesai XT32 LiDAR with 50m range and 30 degree narrow field of view to map Evo and Stein am Rhein, and a Hesai QT64 with 30m range, 100 degree wide field of view to map Wytham Woods.} 

% \mfallon{you need to say what model of lidar, what you mean by narrow FoV and also comment on range}
...
In \figref{fig:heatmap_evo12}, we present a heatmap of the descriptor distances between all query and database scans computed under a specific $\tau_{s}$ of  $F_1$-max score. It offers an illustrative visual representation of the discriminative potential of each descriptor. Consistent with the precision-recall curves, Logg3dNet descriptors exhibit high consistency with the ground truth heatmap (highlighted in white circles), indicating a high true positive rate and low false-positive rate respectively. This implies that Logg3dNet descriptors can effectively detect corresponding loop-candidates during revisits, whereas EgoNN and ScanContext tend to be less discriminative, often returning numerous false-positive candidates.  Based on this evidence, we choose Logg3dNet as the place recognition module in our system for the rest of the experiments.
...
\begin{figure}[t]
  ...
  \label{fig:pr_curves}
\end{figure}
\mfallon{technically you tested with data from 4 countries - Australia!}
...
\begin{figure}[t]
  ...
  \caption{Loop closures distribution by distance and angle at various stages of the pipeline.
   ...
   \haedam{height should be reduced}}
...
In this experiment, we showcase the ability of our approach to obtain loop closures between different mapping missions and to merge those missions into a common map. In this scenario, unlike online-SLAM, we employ stringent descriptor thresholds ($\tau_{s}$) to highly reduce the number of false positive loop candidates. \mfallon{why is odometry not available? We literally do store it to file! Reviewers would find the comment confusing}
