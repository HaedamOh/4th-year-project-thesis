...
% 2.3 Pairwise Cycle consistency
Lastly, we carry out a \emph{cycle consistency} verification (\figref{fig:cycle-consistency}), which checks whether the relative transformations between pairs of nodes are mutually consistent with one another. Given four pose graph nodes $n_i, n_j, n_k, n_l$, we test how close the following equivalence holds:
\begin{equation}
...
\begin{algorithm}
  \caption{Pairwise Check Loop Closure}
  \begin{algorithmic}[1]
  \Function{PairwiseCheckLoopClosure}{$\Delta\mathbf{T}_{i,j}, \text{odom\_pose}$}
      \If{last\_corrected\_pose is None}
          \State last\_corrected\_pose $\gets$ $\Delta\mathbf{T}_{i,j}$
          \State last\_odom\_pose $\gets$ odom\_pose
          \State return False          \Comment{First loop closure. Don't publish yet}

      \EndIf
      \State compute delta\_map\_pose and delta\_odom\_pose
      \If{delta\_odom\_pose is too close}
          \State return False
          \Comment{Delta odom check failed - too close to last pose}
      \EndIf
      \If{not check\_similarity\_poses(delta\_map\_pose, delta\_odom\_pose)}
          \If{fail\_counter > max\_fails}
              \State return False
              \Comment{Too many fails. Resetting. Using latest match as initial}
          \ElsIf{delta\_odom\_pose is too far}
              \State return False
              \Comment{Delta odom is too far}
          \Else
              \State return False
              \Comment{Not consistent. Increment fail counter}
          \EndIf
      \Else
          \State fail\_counter $\gets$ 0
          \State update last\_corrected\_pose and last\_odom\_pose
          \State return True
      \EndIf
  \EndFunction
  \end{algorithmic}
  ...
  \begin{algorithm}
  \caption{Check Similarity Poses}
  \begin{algorithmic}[1]
  \Function{CheckSimilarityPoses}{$\text{delta\_map\_pose}, \text{delta\_odom\_pose}$}
      \State convert rotation matrices to quaternions
      \State compute differences in translation and rotation
      \If{any difference exceeds thresholds}
          \State return False
          \Comment{Translation or rotation fail}
      \EndIf
      \State return True
      \Comment{Success! Pose similarity within thresholds}
  \EndFunction
  \end{algorithmic}
  \end{algorithm}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\columnwidth]{pics/factor_graph_v2}
